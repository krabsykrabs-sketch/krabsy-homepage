...
const GAMES = [
  { key: 'verb_slash',      slug: 'verb-slash',      thumb: 'VerbSlash.png' },
  { key: 'crab_slash',      slug: 'crab-slash',      thumb: null },
  { key: 'air_control',     slug: 'air-control',     thumb: null },
  { key: 'verb_snake',      slug: 'verb-snake',      thumb: 'verb-snake.png' },
  { key: 'verb_platformer', slug: 'verb-platformer', thumb: null },
];
...
function buildTabBar(items, currentKey, namespace, strings, ariaLabel, guardActiveDrill) {
  ...
  for (const item of items) {
    ...
    if (isCurrent) {
      ...
      // namespace is either 'drills' or 'tools' (matches the URL segment)
      const href = `/${lang}/${namespace}/${item.slug}/`;
      a.href = href;
      if (guardActiveDrill) {
...
function buildGameStrip(strings, count) {
  ...
  for (const g of picked) {
    ...
    a.href = `/${currentLang()}/games/${g.slug}/`;
