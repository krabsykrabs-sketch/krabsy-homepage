...
/**
 * Record one answer.
 *   itemId  — opaque per-topic identifier; for irregular verbs, the base form
 *             (e.g. "fly", "go"). Renamed from verbId for forward-compat.
 ...
 *
 * NOTE: `topic` is accepted but not yet persisted (v1 schema is single-topic).
 * When v2 schema lands (multi-topic), this argument starts being stored per attempt.
 * Do not remove this parameter — drills/tools already pass it.
 */
export function recordAttempt(itemId, form, correct, source, topic = 'irregular_verbs') {
  ...
  // Per-(item, form) record. Stored under `verbs` in the v1 single-topic
  // schema; renamed to a topic-aware structure when v2 lands.
  if (!s.verbs[itemId]) s.verbs[itemId] = {};
  if (!s.verbs[itemId][form]) {
    s.verbs[itemId][form] = { attempts: 0, correct: 0, mastery: 0, last_seen: now };
  }
  const rec = s.verbs[itemId][form];
  rec.attempts += 1;
  rec.last_seen = now;
  if (correct) {
    rec.correct += 1;
