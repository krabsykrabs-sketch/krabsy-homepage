...
export function recordAttempt(itemId, form, correct, source, topic = DEFAULT_TOPIC) {
  if (!itemId) return;
  // The `form` value is set by data adapters in krabsy-data.js, not by
  // user input. Each topic chooses its own form set: irregular_verbs uses
  // 'past'/'participle', prepositions uses 'default', and future topics
  // (phrasal verbs, conditionals, etc.) will define their own. Accepting
  // any non-empty string lets topics extend the stats schema without
  // touching this file. Typo risk is bounded by code review at the
  // adapter-add site.
...
/**
 * Per-use-case mastery for prepositions. Reads the 'default' form (the
 * single form prepositions track) and returns 0 when no record exists.
 * Used by the future preposition reference page for the mastery overlay.
 */
export function prepositionMastery(useCaseId, topic = 'prepositions') {
  const v = getItemStats(useCaseId, topic);
  if (!v) return 0;
  return (v.default && v.default.mastery) || 0;
}
